...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        let path_for_name = format!("/proc/{}/fd/{}", pid, fd);
        let name = OpenFile::path_to_name(fs::read_link(path_for_name).ok()?.to_str()?);

        let path_for_cursor = format!("/proc/{}/fdinfo/{}", pid, fd);
        let string = fs::read_to_string(path_for_cursor).ok()?;
        let cursor = OpenFile::parse_cursor(&string)?;
        let access_mode = OpenFile::parse_access_mode(&string)?;
